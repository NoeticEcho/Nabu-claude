...
### Шаг 12 — Membership-гейт групповых чатов (решение пользователя: добавить allowlist/гейт)
**Проблема:** много-тенантный chat-allow пускает ЛЮБУЮ группу → авто-провижининг проекта + трата квоты Claude.
**Что меняю:** `cli/telegram-bot.mjs` — групповой чат обслуживается ТОЛЬКО если выполнено одно из:
(а) проектное пространство для `chat.id` уже существует; (б) автор реплики (`msg.from`) — уже
зарегистрированный пользователь (`resolveTenantByTelegram(..., {autoRegister:false})` ≠ null); (в) `chat.id`
в явном allowlist (`NABU_GROUP_ALLOWLIST`, csv). Иначе — не отвечать (и не создавать space/аккаунт).
Это привязывает создание проектного пространства к известному пользователю, а не к случайному добавлению бота.
**Файлы:** `cli/telegram-bot.mjs` (+ резолвинг: не авто-создавать group space, пока гейт не пройден).
**Проверка:** B, T + **тест**: незнакомая группа без зарегистрированного автора → отказ, space не создан;
группа с зарегистрированным автором / в allowlist → обслуживается.

---

## Осознанно ВНЕ объёма (с причиной)

- **orphan-kill ppid-фильтр (nabu.mjs:497)** — пользователь подтвердил **single-instance** модель OlimpOS
  (один демон на хост), поэтому текущее поведение корректно; правка не нужна.
