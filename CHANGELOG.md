...
## [1.13.0] — 2026-07-07
### Added
- **OlimpOS — много-тенантный режим (опционально, `NABU_MULTITENANT=1`).** Один инстанс Nabu на команду:
  - **Изоляция по пользователю**: у каждого своё пространство `u:<userId>`; личная память/знания/задачи
    НИКОГДА не смешиваются между пользователями (двух-осевой скоуп namespace + user_id, fail-closed).
  - **Веб-аутентификация**: регистрация/вход по email+паролю (scrypt) **или** через Telegram (deep-link,
    одноразовый код). Сессии подписаны HMAC, `HttpOnly`-cookie.
  - **Групповые Telegram-чаты = проектные пространства** (`g:<chatId>`) с атрибуцией по автору; общая
    память/задачи проекта изолированы от личных. Membership-гейт (`NABU_GROUP_ALLOWLIST`/зарегистрированный автор).
  - **Agile-доска**: эпики, спринты, kanban (todo/doing/review/done), оценки, velocity/burndown.
  - **Docker-песочницы проектов**: изолированный запуск кода (без сети/секретов/хоста по умолчанию,
    лимиты, под uid демона), git clone/commit/status; **`git push` — через approval** (одноразовый, маскировка токена).
  - **Spaces**: публикация статических сайтов из Markdown на `/s/<slug>` (встроенный генератор).
  - **Рынок агентов** (`__commons__`): публикация своих агентов/скиллов/опыта в общий банк.
  - **Веб-панель управления** (🛠) + новый MCP-сервер **nabu-olimpos** (22 инструмента P3–P7).
  - Дефолтный одно-пользовательский локальный режим (localhost-trust) **не меняется**.
- **Public API v1** (`/api/v1`) — токен-аутентифицированный REST для внешних клиентов (мобильное
  приложение, скрипты). Bearer personal-access-токены (`nabu_pat_…`, в БД sha256-хеш, отзыв), строгий
  скоуп по пользователю. Эндпоинты: chat (sync + SSE), memory/recall (без vault), tasks/projects,
  olimpos board/agents, conversations, me. Работает в много-тенанте (token→user) и локально
  (`NABU_API_TOKEN`). Управление токенами — из веб-панели или `POST /api/v1/auth/token`. (docs/API.md.)
- **Гайды**: `docs/OLIMPOS_GUIDE.md` (пользователь), `docs/DEPLOY_HTTPS.md` (VPS за Caddy/nginx),
  `docs/OLIMPOS_ROADMAP.md`, `docs/API.md`.
- **Демон**: флаг `NABU_DAEMON_ALLOW_DANGEROUS` (включить Bash/Edit/Workflow адъютанту в песочнице),
  настраиваемый таймаут хода claude (`NABU_CLAUDE_TIMEOUT_MS`, дефолт 30 мин), crisis-eval golden-фикстуры + CI-гейт.

### Security
- **Аудит Round 8** нового OlimpOS-кода (`AUDIT.md`). Исправлено 1 critical + 5 major + minor-хардненинг:
  - **Fail-closed резолвинг тенанта**: при сбое БД в много-тенанте обмен отклоняется, а не деградирует
    к пространству владельца демона (была cross-tenant утечка памяти/сессии).
  - **Скоуп публикации агента по владельцу** (`shareAgent`) — нельзя опубликовать чужого приватного агента.
  - **Telegram login-код одноразовый + TTL** (защита от перехвата/перепривязки к чужому аккаунту).
  - **Sandbox**: закрыт symlink-traversal на хост; таймаут реально убивает контейнер; контейнер под uid демона.
  - **web-auth**: `Secure`-cookie в много-тенанте, rate-limit на логин/регистрацию, предупреждение о дефолтном секрете.
  - Валидация API-эндпоинтов, `publishSpace` не глушит ошибки БД, SVG публичных сайтов → text/plain, defense-in-depth путей.

### Fixed
- **Гонка claude-сессий**: старт демона добивает осиротевшие claude-процессы (корень дублей ходов после хард-рестарта).
- Мелкие консистентности: динамическая строка таймаута, точный счётчик посева агентов, синхронизация TTL-констант.
...
- **Индексация папок с прогрессом + ZIP.** `nabu index <папка|архив.zip> [--domain= --library]` —
  рекурсивный обход (текст/md/csv/json + PDF через pdftotext), песочница (~/ и NABU_HOME), прогресс.
  В Telegram: загрузка `.zip` → автораспаковка → ФОНОВАЯ индексация с прогрессом в топик и итогом;
  во время индексации можно продолжать общаться с адъютантом/министрами (проверено). Общий модуль
  `lib/folder-index.ts` (`indexFolder` с колбэком прогресса).
...
- **`nabu-council.list_deliberations`** (read-only) — подъём прошлых наработок Совета (совещания с
  синтезом/trade-off'ами/решениями) в новый разговор. Раньше tool'а не было → адъютант ошибочно
  сообщал, что это «требует подтверждения прав».
- **Telegram reply/цитата**: ответ на сообщение (reply) или выделенный фрагмент подставляется в
  контекст — адъютант понимает, что именно цитирует пользователь и кто это сказал.
### Fixed
- SKILL: явное правило — ВСЕ операции чтения (recall/list_*/get_*) автономны, никогда не выдаются
  за «требует подтверждения прав»; approval только для высокорисковых write-действий.

## [1.8.1] — 2026-07-05
### Fixed
- **Файлы от адъютанта реально отправляются**: захват из рабочей папки процесса (repoRoot — куда
  адъютант пишет относительными именами), отправка (Telegram-документ / веб-ссылка) и перенос в
  workspace, чтобы код не засорялся. SSE-событие `files` шлётся ДО `done` (иначе поток закрыт).
- **Форматирование на длинных ответах**: HTML-рендер применяется к КАЖДОМУ сегменту разбитого
  ответа, а не только к последнему.
- Откат регрессии cwd (Claude Code headless в недоверенной папке давал пустой ответ).

## [1.8.0] — 2026-07-05
### Added
- **Форматирование в Telegram**: ответы рендерятся (markdown → Telegram HTML: жирный/курсив/код/
  ссылки/списки) вместо сырых звёздочек; веб-рендер расширен до того же набора.
- **Файлы в Telegram**: приём документов (md/txt/csv/json/код/PDF → содержимое как сообщение или
  заметка), отправка файлов пользователю (адъютант пишет в `~/nabu/.nabu/outbox/` → бот шлёт).
