...
## Этап 5: Статус исправлений (2026-07-07)

Рефакторинг выполнен по `REFACTOR_PLAN.md` (пользователь подтвердил **весь план 1–11 + Шаг 12**).
**12 атомарных коммитов** на master. После каждого: `tsc -b` + `npm test` (87) + целевой тест шага.
Финал: build 0 · **87 unit** · **70 guard + 28 web-privacy** · olimpos MCP 22 tools ·
**6 интеграционных наборов R8 против живой БД/docker (28 проверок) — все зелёные**.

### ✅ Исправлено (1 critical + 5 major + все minor-батчи)

| Находка | Как закрыто | Проверка |
|---|---|---|
| **C1** fail-open резолвинг тенанта | telegram-bot: в много-тенанте пустой/ошибочный резолвинг → **отказ в обмене** (не деградируем к scope владельца + общей сессии) | node --check + 87 unit |
| **M1** shareAgent кросс-тенант | `and (origin_user=$2 or origin_user is null)` в WHERE; пустой owner → false | 6/6 integ (A не шарит B) |
| **M2** login-код не одноразовый | claim только если `tg_user_id is null` и свежий (TTL) — атомарно в UPDATE | 7/7 integ (перехват отклонён) |
| **M3** publishSpace глушит ошибки | `.catch(()=>{})` → `returning`-проверка; реальная ошибка БД пробрасывается | 4/4 integ |
| **M4** sandbox symlink-traversal | `withinWorkdir` сверяет realpath глубочайшего существующего предка цели | 5/5 integ (symlink наружу отвергнут) |
| **M5** sandbox timeout не убивает контейнер | `--name` + `docker rm -f` по таймауту (SIGKILL бил лишь клиент) | 3/3 integ (docker, ~3с) |
| **Шаг 7** hardening песочницы | maskCreds в clone/status/commit; dockerAvailable-гвард; docker-check до потребления approval; контейнер под uid демона (не root) | 3/3 integ + MCP 22 |
| **Шаг 8** пути+SVG | resolveSitePath/safeSitePath отвергают `.`/`..`, мёртвый target убран; SVG → text/plain | dot-slug/traversal отвергнуты |
| **Шаг 9** web-auth | Secure-cookie в много-тенанте; rate-limit login/register(8/мин)+poll(60/мин)→429; warn о дефолт-секрете | 3/3 (429, Secure, warn) |
| **Шаг 10** валидация API | task-move/estimate валидируют taskId/column/points→400; catch не светит сырой текст (лог на сервер); sprintMetrics клампит column; esc() числовых полей UI | 3/3 (400) |
| **Шаг 11** косметика | tg timeout-строка динамическая; seedBuiltinAgents реальный счётчик; TTL-константы синхронизированы; 019 комментарий scrypt | seed 2→0 |
| **Шаг 12** membership-гейт групп | группа обслуживается только если space существует / автор зарегистрирован / chat.id в `NABU_GROUP_ALLOWLIST`; иначе отказ, space не создаётся | примитивы гейта |

### ⏸ Отложено (осознанно, с причиной)

- **resolveGroupSpace backfill account_user (minor)** — null-`account_user` предотвращается транзакцией
  создания (аккаунт+space в одном tx); состояние в норме недостижимо. Правка low-value и рискует
  задеть рабочий путь — оставил, помечено здесь.
- **consumeLoginCode TOCTOU (minor)** — тот же пользователь, не кросс-тенант; окно сузилось после M2
  (claim теперь одноразовый), consume и так удаляет код. Не критично.
- **timestamptz-драйвер TTL (minor)** — драйвер возвращает timestamptz со смещением (проверено на
  живой БД: TTL-тесты M2/M3 корректны), правка не требуется.

### ⚠ Требует решения/действия пользователя (риски)

1. **Группы-allowlist политика** — гейт добавлен (Шаг 12); если нужен полностью открытый публичный бот
   ИЛИ, наоборот, строгий allowlist по chat.id — задай `NABU_GROUP_ALLOWLIST` или скажи, ослабить/ужесточить.
2. **Секрет сессий в проде** — на VPS **обязательно** задать `NABU_SESSION_SECRET` (или `NABU_VAULT_KEY`);
   при дефолте сервер теперь громко предупреждает, но не падает.
3. **Мониторинг под реальной много-тенантной нагрузкой** — C1/M-фиксы проверены интеграционно на happy-
   и fail-путях; понаблюдай при живом одновременном использовании несколькими пользователями (web+TG).

*Round 8 закрыт. R7 — в `AUDIT_R7_archive.md`. Все изменения на master (12 коммитов рефакторинга + аудит/план).*
