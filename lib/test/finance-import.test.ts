...
test("parseAmount: пробелы/nbsp тысячники, запятая и точка десятичные", () => {
  ...
  // R7-E9: неоднозначность одиночной запятой
  assert.equal(parseAmount("1,000"), 1000);       // запятая + 3 цифры → тысячи, НЕ 1.0
  assert.equal(parseAmount("1,000,000"), 1000000); // несколько запятых → все тысячи
  assert.equal(parseAmount("1,50"), 1.5);          // 2 цифры → десятичная
