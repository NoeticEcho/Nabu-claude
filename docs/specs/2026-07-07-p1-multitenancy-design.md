...
## 5a. ⚠️ КРИТИЧНО (найдено при wiring): разговоры/сессии — per-tenant

`cid = convId(role)` даёт `conv-adjutant` — ОДИН на всех. В много-тенантном режиме это привело бы к
общей сессии/истории между пользователями (**утечка**). Требование: в `NABU_MULTITENANT=1` идентификатор
разговора и `sessionKey` ДОЛЖНЫ включать тенанта — `conv-<role>-<userId>`. Также `withConversationLock`
ключуется этим составным id (сериализация в рамках тенанта, параллельность между тенантами). Механизм
готов: `runClaude`/`runClaudeExchange` принимают `extraEnv` ({NABU_NAMESPACE, NABU_USER_ID}) — MCP
скоупятся к тенанту. Остаётся: (а) резолвер на входе (msg.from.id → tenant), (б) tenant-aware cid,
(в) снятие единого boundChatId. Делать аккуратно с реальным мульти-юзер тестом (не наспех — риск leak).
